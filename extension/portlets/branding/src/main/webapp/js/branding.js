$(function() {
	...
	/*
	 * $("#navigationStyle").jzAjax({ url : "BrandingController.save()", data : {
	 * "style" : $('#navigationStyle option:selected').val(), "isChangeLogo" :
	 * isChangeLogo }, beforeSend : function() { $("#saveinfo").hide(); },
	 * success : function(data) { isChangeLogo = false;
	 * UpdateTopBarNavigation(data); cleanMessage(); $("#saveinfo").show(); }
	 * });
	 */
	});
	...
	function UpdatePreviewLogoAndStyle() {
		...
		/*
		 * $("#navigationStyle").jzAjax( { url :
		 * "BrandingController.getResource()", beforeSend : function() { },
		 * success : function(data) { // update the logo url in preview zone and
		 * preview // navigation bar previewLogoFromUrl(data.logoUrl); // update
		 * the navigation style and style selected;
		 * changePreviewStyle(data.style);
